...
def weekly_contest_386_solution_2(bottomLeft: List[List[int]], topRight: List[List[int]]) -> int:
    # WA
    def get_common(k, m):
        area1 = areas[k]
        area2 = areas[m]
        # 包裹
        if area1[1][0] <= area2[0][0]:  # 2 在 1 右侧
            return 0
        if area1[1][1] <= area2[0][1]:  # 2在1上侧
            return 0
        if area1[0][1] >= area2[1][1]:  # 2在1 下侧
            return 0
        d_x = - (area2[0][0] - min(area2[1][0], area1[1][0]))
        if area2[1][1] > area1[1][1]:
            d_y = area1[1][1] - area2[0][1]
        else:
            d_y = area2[1][1] - max(area1[0][1], area2[0][1])
        return min(d_x, d_y) ** 2

    max_area = 0
    n = len(bottomLeft)
    areas = list(zip(bottomLeft, topRight))
    areas.sort(key=lambda x: (x[0][0], x[1][0], x[0][1], x[1][1]))
    for i in range(n):
        for j in range(i + 1, n):
            max_area = max(max_area, get_common(i, j))
    return max_area


def weekly_contest_386_solution_3(nums: List[int], changeIndices: List[int]) -> int:
    m = len(changeIndices)
    n = len(nums)
    changeIndices = [x - 1 for x in changeIndices]
    cnt = Counter[int](changeIndices)
    for i, num in enumerate(nums):
        if cnt[i] < num + 1:
            return -1
    f = {c: idx for idx, c in enumerate(changeIndices)}


def weekly_contest_386_solution_4():
    ...
